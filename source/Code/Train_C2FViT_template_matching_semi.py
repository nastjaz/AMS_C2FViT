...
from C2FViT_model import C2F_ViT_stage, AffineCOMTransform, Center_of_mass_initial_pairwise, multi_resolution_NCC, MulticlassDiceLossVectorize
from Functions import Dataset_epoch_MNI152, Dataset_epoch_MNI152_pre_one_hot
...
def train():
    ...
    training_generator = Data.DataLoader(Dataset_epoch_MNI152_pre_one_hot(imgs, labels, MNI152_img, MNI152_label, need_label=True),
                                         batch_size=1,
    ...
    while step <= iteration:
        for X, Y, X_label, Y_label in training_generator:
            ...
            if (step % n_checkpoint == 0):
                ...
                # ---------------------------------------
                # imgs = sorted(glob.glob(datapath + "/OASIS_OAS1_*_MR1/norm.nii.gz"))[255:260]
                # labels = sorted(glob.glob(datapath + "/OASIS_OAS1_*_MR1/seg4_mni.nii.gz"))[255:260]
                # MNI152_img = "../Data/MNI152_T1_1mm_brain_pad_RSP.nii.gz"
                ...
                #         F_X_Y = F.affine_grid(affine_matrix, X_label.shape, align_corners=True)
                #
                #         X_Y_label = F.grid_sample(X_label, F_X_Y, mode="nearest", align_corners=True).cpu().numpy()[0, 0, :, :, :]
